...
const summaryList = Object.entries(groupedRewards).map(([payee, payeeRewards]) => {
  const reward = payeeRewards!.reduce(
    (acc, { currency, reward }) => {
      acc[currency] ??= 0
      acc[currency] += reward
      return acc
  ...
  return {
    payee,
    reward,
    accounts: payeeRewards!.map(({ payee: _, ...account }) => account),
  }
})
