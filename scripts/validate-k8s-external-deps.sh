...
CONFIGMAP_FILE=${1:-$ROOT_DIR/deploy/k8s/base/configmap.yaml}
...
sourceid_authorization_uri=$(yaml_value "$CONFIGMAP_FILE" "oauth2-sourceid-authorization-uri")
sourceid_token_uri=$(yaml_value "$CONFIGMAP_FILE" "oauth2-sourceid-token-uri")
sourceid_user_info_uri=$(yaml_value "$CONFIGMAP_FILE" "oauth2-sourceid-user-info-uri")
...
require_non_empty postgres-host "$postgres_host"
require_non_empty postgres-port "$postgres_port"
require_non_empty postgres-db "$postgres_db"
require_non_empty postgres-user "$postgres_user"
...
reject_placeholder postgres-host "$postgres_host" "postgres"
reject_placeholder redis-host "$redis_host" "redis"
...
reject_placeholder BOOTSTRAP_ADMIN_PASSWORD "$bootstrap_admin_password" "change-me" "ChangeMe!2026" "Admin@2026"
...
case "$storage_provider" in
  s3)
    ...
    ;;
  local)
    warn "skillhub-storage-provider=local is not recommended for external Kubernetes deployments"
    ;;
  *)
...
if [[ -n "$sourceid_redirect_uri" || -n "$sourceid_authorization_uri" || -n "$sourceid_token_uri" || -n "$sourceid_user_info_uri" ]]; then
  sourceid_enabled=true
fi
if [[ -n "$sourceid_client_id" && "$sourceid_client_id" != "change-me" ]]; then
  sourceid_enabled=true
fi
if [[ -n "$sourceid_client_secret" && "$sourceid_client_secret" != "change-me" ]]; then
  sourceid_enabled=true
fi
...
if [[ "$sourceid_enabled" == "true" ]]; then
  ...
  require_non_empty oauth2-sourceid-redirect-uri "$sourceid_redirect_uri"
  require_non_empty oauth2-sourceid-authorization-uri "$sourceid_authorization_uri"
  require_non_empty oauth2-sourceid-token-uri "$sourceid_token_uri"
  require_non_empty oauth2-sourceid-user-info-uri "$sourceid_user_info_uri"
  validate_url oauth2-sourceid-redirect-uri "$sourceid_redirect_uri"
  validate_url oauth2-sourceid-authorization-uri "$sourceid_authorization_uri"
  validate_url oauth2-sourceid-token-uri "$sourceid_token_uri"
  validate_url oauth2-sourceid-user-info-uri "$sourceid_user_info_uri"
