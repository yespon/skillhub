---
name: backend
description: Use when implementing or modifying SkillHub backend Java code under `server/`, especially Spring Boot, layering, Controller boundaries, and Flyway-related work.
model: inherit
---
...
- 新 migration 文件版本号必须连续递增，以当前工作树中最高的 `V*.sql` 为准
- **禁止**修改已有 `V*.sql` 文件内容
